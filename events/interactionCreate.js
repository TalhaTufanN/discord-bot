        ...
        switch (customId) {
          ...
          case 'support':
            await interaction.reply({ 
              embeds: [infoEmbed('Bot ile ilgili yardım almak için destek sunucumuza katılın!')], 
              ephemeral: true 
            });
            break;
          ...
          case 'invite':
            await interaction.reply({ 
              embeds: [infoEmbed('Botu sunucunuza davet etmek istediğiniz için teşekkürler!')], 
              ephemeral: true 
            });
            break;
          ...
          case 'music_loop': {
            ...
            switch (customId) {
              case 'music_pause_resume':
                ...
                const pauseButton = buttonRow.components.find(c => c.data.custom_id === 'music_pause_resume');
              ...
              case 'music_shuffle':
                 queue.shuffle();
                 await interaction.reply({ embeds: [infoEmbed('Kuyruk karıştırıldı.')], ephemeral: true });
                 break;
              ...
              case 'music_loop':
                 ...
                 await interaction.reply({ 
                     embeds: [infoEmbed(`Döngü modu: ${mode === 2 ? 'Kuyruk' : 'Kapalı'}`)], 
                     ephemeral: true 
                 });
                 break;
              ...
              case 'music_stop':
                queue.stop();
                await interaction.reply({ embeds: [infoEmbed('Müzik durduruldu ve kuyruk temizlendi.')], ephemeral: true });
                break;
              ...
              case 'music_previous':
                try {
                  ...
                  await interaction.reply({ embeds: [errorEmbed('Önceki şarkı bulunamadı!')], ephemeral: true });
                }
                break;
              ...
              case 'music_skip':
                if (queue.songs.length > 1 || queue.autoplay) {
                  try {
                    ...
                    queue.stop();
                    await interaction.reply({ embeds: [infoEmbed('Sırada şarkı olmadığı için müzik sonlandırıldı.')], ephemeral: true });
                  }
                } else {
                  queue.stop();
                  await interaction.reply({ embeds: [infoEmbed('Sırada şarkı olmadığı için müzik sonlandırıldı.')], ephemeral: true });
          ...
          case 'music_leave': {
            ...
            await interaction.reply({ embeds: [infoEmbed('Kanaldan ayrıldım. 👋')], ephemeral: true });
            break;
          }
